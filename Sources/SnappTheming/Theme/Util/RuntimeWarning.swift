...
internal func runtimeWarning(_ message: String) {
        ...
        if runningTests() {
            ...
            dladdr(
                dlsym(
                    dlopen(nil, RTLD_LAZY), "$s10Foundation15AttributeScopesO7SwiftUIE05swiftE0AcDE0D12UIAttributesVmvg"
                ), &info)
            ...
            os_log(
                .fault, dso: info.dli_fbase,
                log: OSLog(subsystem: "com.apple.runtime-issues", category: "SnappTheming"), "%@", message)
        } else {
