...
struct SAThemingTokenResolver<Value> where Value: Codable {
    /// A dictionary holding the base tokens, organized by component and name.
    let baseValues: [String: [String: SAThemingToken<Value>]]
    ...
    func resolve(_ token: SAThemingToken<Value>) -> Value? {
        resolve(token, from: [])
    }
    ...
    private func resolve(_ token: SAThemingToken<Value>, from visitedPaths: [SAThemingTokenPath]) -> Value? {
        switch token {
