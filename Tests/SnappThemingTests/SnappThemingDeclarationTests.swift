    ...
    final class SnappThemingDeclarationTests {
        ...
        @Test func overrideDeclaration() async throws {
            ...
            let overrideJSON = """
                ...
                """
            let baseDeclaration = try SnappThemingParser.parse(
                from: baseJSON, using: configuration)
            let overrideDeclaration = try SnappThemingParser.parse(
                from: overrideJSON, using: configuration)

            let declaration = baseDeclaration.override(
                with: overrideDeclaration, using: configuration)
